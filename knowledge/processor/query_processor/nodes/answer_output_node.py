...
class AnswerOutputNode(BaseNode):
    ...
    MINIO_URL_PREFIX = "http://192.168.27.110:9000/knowledge-base-v1/"
    PROXY_URL_PREFIX = "/images/"
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        if answer:
            ...
            #4.1.3 将 MinIO 内网地址替换为后端代理地址
            llm_answer = self._replace_minio_url(llm_answer)
            state["answer"] = llm_answer
            #4.1.4 流式输出完之后，最终要告诉前端 SSE通道关闭了
            if is_stream :
    ...
    def _replace_minio_url(self, text: str) -> str:
        """将 MinIO 内网地址替换为后端代理地址"""
        if not text:
            return text
        return text.replace(self.MINIO_URL_PREFIX, self.PROXY_URL_PREFIX)
