    ...
    async def mcp_web_search(self,rewritten_query:str):
        ...
        async with MCPServerStreamableHttp(
                ...
                params={
                    ...
                    "headers": {"Authorization": "Bearer ms-5dbb868c-c7ff-4dc3-8456-af735bfb38cf"},  # 认证头
            ...
            self.logger.info(f"MCP服务可用工具列表: {tools}")
            for tool in tools:
                self.logger.info(f"工具名: {tool.name}, 描述: {tool.description}, 参数: {tool.inputSchema}")
            ...
            self.logger.info(f"call_tool返回结果: {execute_tool_result}")
            self.logger.info(f"content: {execute_tool_result.content}")
